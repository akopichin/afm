...
**Re-run с учётом технологий (2026-07-05).** Первый прогон (`goga-apply`, коммит `dad0e7c`) выполнялся
до `goga init` — `.goga/config.yml` ещё не объявлял `language: golang`, `.goga/usages/conventions.md` не
существовал. Теперь конфигурация подключена, и план дополнен: каждая из 21 conformance-задачи (`[verify/conform]`)
получила шаг **STEP 2b (CONVENTIONS CHECK)** — явную сверку кода клеточки с `.goga/usages/conventions.md`
(именованные импорты, DI через конструктор, `context.Context` первым параметром, `errors.Is`/`errors.As`+`%w`,
`log/slog`, bounded concurrency, table-driven тесты). Это по-прежнему conformance-audit поверх существующей
архитектуры — границы клеточек и CODEMANIFEST-контракты не меняются, расхождение с конвенциями правится
только в коде.
...
- **STEP 1 CONTRACT TESTS** — проверка доступности фасада + объявленной формы API против `CODEMANIFEST`
  (как правило уже покрыто существующим suite'ом из 37 `_test.go`-файлов; где нет — добавить
...
- **STEP 2b CONVENTIONS CHECK** — сверить код клеточки с `.goga/usages/conventions.md`; расхождение
  фиксируется как gap и правится в коде (контракт не трогается);
...
14 cell-level `.usages/`-файлов существуют и свёрены с оттрассированными контрактами — состав API
потребителя не менялся (as-is), дополнений не требуется. Создание/обновление в этом плане **не требуется**
(status: current). 13 из них созданы стадией `goga-apply` (первый прогон, коммит `dad0e7c`); 14-й,
`pkg/progress/.usages/logger_facade.md`, материализован позже, повторным `goga-apply` (2026-07-05), —
закрыл пробел, найденный аудитом `brainstorm`/`brainstorm-review` (`pkg/progress` имел внутреннего
потребителя `pkg/executor`, но не имел cell-level `.usages/`). Ни одна клеточка не ссылается в
`CODEMANIFEST Usages` на собственный `.usages/`. Файлы:
...
- `pkg/progress/.usages/logger_facade.md` (facade-документация, единственный потребитель `pkg/executor`).
...
- **37 `_test.go`-файлов** уже существуют (orchestrator 14, cmd/afm 5, server 3, proxy 3, state 2, prompts 2,
  executor 2, config/flow/progress/mcp/docker/web по 1; `pkg/web/dashboard`/`assets`/`tools/setstatuslinter` — 0).
  **Коррекция:** `pkg/web/embed_test.go` уже существует (`TestEmbed_ServesOriginalWebPathsAfterDirMove`) —
  более ранняя версия этого плана ошибочно считала `pkg/web` непокрытым; см. Gap Analysis и Task 12.
...
- **Existing code that can be reused**: весь — это реверс-инжиниринг as-is; suite из 37 файлов переиспользуется.
- **Test coverage gaps**: нет подтверждённых пробелов — все 8 сценариев дизайн-документа имеют эквиваленты
  в существующем suite (37 `_test.go`-файлов) → задачи их верифицируют+документируют (где имя не совпадает
  дословно — задача сверяет поведение и при желании добавляет алиас-тест с каноническим именем).
  - **Коррекция (эта ревизия плана):** `pkg/web` **не** является пробелом — живая проверка кода показала, что
    `pkg/web/embed_test.go` уже содержит `TestEmbed_ServesOriginalWebPathsAfterDirMove`, покрывающий ровно
    сценарий design-дока (все 5 ассетов доступны по корневым путям после переноса в `pkg/web/dashboard/`,
    плюс проверка, что префикс `dashboard/` не протекает). Более ранняя версия этого плана (до этой ревизии)
    ошибочно утверждала «0 тестов» и требовала создать новый тест — исправлено; Task 11/12 теперь
    верифицируют существующий тест, а не создают новый.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/config` против `.goga/usages/conventions.md`: именованные импорты (без dot-import), error wrapping через `fmt.Errorf("...: %w", err)` в `LoadFrom`, отсутствие service-locator/reflection-DI (простые функции — DI не требуется). Расхождение — править код, не контракт.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/flow` против `.goga/usages/conventions.md`: именованные импорты, error wrapping в `ParseFile` (`%w`), table-driven тесты для двух форм `Input.UnmarshalYAML`. Расхождение — править код, не контракт.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/state` против `.goga/usages/conventions.md`: именованные импорты, error wrapping (`errors.Is`/`errors.As`+`%w`) в `Store.Apply`/fsync-путях, отсутствие implicit DI (явные параметры конструктора у `Store`). Расхождение — править код, не контракт.
...
- `logger_facade.md`: фасад потребителя (`pkg/executor`) — как вызывать `NewLogger`/`Log`/`LogAction`/`LogStart`/`LogEnd`/`Close`.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/progress` против `.goga/usages/conventions.md`: именованные импорты, использование `log/slog` (или обоснованное исключение для append-only `Logger`, если формат логов зафиксирован контрактом), платформенные build-tags не нарушают явность контроля потока. Расхождение — править код, не контракт.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/mcp` против `.goga/usages/conventions.md`: именованные импорты, error wrapping в `ReadDialog`/`AppendAnswer` (`%w`), сериализация конкурентного доступа (`appendMu`) соответствует bounded-concurrency духу конвенций, table-driven тесты для `ReadDialog`/`FindEntry`. Расхождение — править код, не контракт.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/proxy` (ZAI transform) против `.goga/usages/conventions.md`: `context.Context` пробрасывается в `http.NewRequestWithContext`, error wrapping (`%w`) при `io.ReadAll`/`json.Marshal`, именованные импорты, table-driven тесты для `parseSSE`. Расхождение — править код, не контракт.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/proxy` (proxy infra + shim) против `.goga/usages/conventions.md`: DI через конструктор (`New(upstream, transforms)` — явные параметры, не service locator), error wrapping в `CreateShim`, именованные импорты. Расхождение — править код, не контракт.
- [ ] **STEP 3 (INTERFACE VERIFICATION)**: `go test ./pkg/proxy/...` — зелёные.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/web/dashboard` против `.goga/usages/conventions.md`: применимо только именованные импорты (сущность — статические ассеты, entities/DI/error-handling конвенции не применимы). Расхождение — править код, не контракт.
...
- [ ] **STEP 1 (CONTRACT TESTS)**: contract-тест на фасад (`web.FS` возвращает `embed.FS`). Уже покрыто
  `pkg/web/embed_test.go` (`TestEmbed_ServesOriginalWebPathsAfterDirMove` проверяет `web.FS` и через него —
  доступность фасада); отдельного shape-теста добавлять не требуется — не создавать новый файл.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/web` против `.goga/usages/conventions.md`: именованные импорты; DI/context/error-handling конвенции не применимы к тривиальному embed-фасаду (`FS()`). Расхождение — править код, не контракт.
- [ ] **STEP 3 (INTERFACE VERIFICATION)**: `go test ./pkg/web/...` — зелёные (существующий тест).
- [ ] **STEP 4 (LOGIC TESTS)**: поведение — `fs.Sub` даёт корневые пути (без префикса `dashboard/`). Покрытие
  поведения — уже в `embed_test.go`, дополнительно верифицируется в Task 12 (integration).
...
### Task 12: `pkg/web` — integration test (embed serves original paths after dir move)  [integration]

Integration-сценарий embed-split, переносимый из дизайн-дока §Test Stack Trace (Edge Case). **Коррекция:**
не genuine gap — уже покрыт `pkg/web/embed_test.go` (`TestEmbed_ServesOriginalWebPathsAfterDirMove`); задача
верифицирует эквивалентность дословному сценарию дизайн-дока, а не создаёт новый тест.
...
- [ ] **Edge: `TestWeb_Embed_ServesOriginalWebPaths_AfterDirMove`** (дословно из дизайн-дока):
  - **Setup**: собранный бинарь (embed `dashboard/*`).
  - **Input**: доступ к `web.FS` по корневым путям (`fs.Stat`/`http.FileServer`).
  - **Trace**: `web.FS = fs.Sub(embedded,"dashboard") → отдаёт ассеты по корневому пути (без префикса dashboard/)`.
  - **Assertions**: `index.html`, `style.css`, `app.js`, `favicon.svg`, `markdown-it.min.js` доступны по корневым путям.
  - **Sufficiency**: регресс на слом embed после физического переноса ассетов в `pkg/web/dashboard/`.
  - Эквивалент в suite: `TestEmbed_ServesOriginalWebPathsAfterDirMove` (`pkg/web/embed_test.go`) — сверить
    поведение (уже проверяет все 5 ассетов через `fs.Stat(web.FS, name)` + непротекание префикса `dashboard/`,
    что строже дословного сценария); при желании добавить алиас `TestWeb_Embed_ServesOriginalWebPaths_AfterDirMove`
    с каноническим именем — необязательно, поведение уже покрыто.
- [ ] **Run validation**: `go test ./pkg/web/... -run 'ServesOriginalWebPathsAfterDirMove'` — зелёный.
- [ ] **Cross-check**: `goga lint` остаётся 16/0, `go build ./...` exit 0.
...
- [ ] **STEP 2 (IMPLEMENTATION)**: проверить `assets.go` — embed-директивы, `ReadPrompt` (override), `FS`/`SkillsFS`.
  Зазоров нет; править код при находке.
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `assets` против `.goga/usages/conventions.md`: именованные импорты, error wrapping в `ReadPrompt` (`%w` при отсутствии override/embed-файла). Расхождение — править код, не контракт.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `tools/setstatuslinter` против `.goga/usages/conventions.md`: именованные импорты, идиоматичное использование `go/analysis` (`singlechecker.Main`) без reflection-хаков. Расхождение — править код, не контракт.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/prompts` против `.goga/usages/conventions.md`: именованные импорты, error handling в `ValidatePlan` (явный возврат `PlanIssues`, не паника), table-driven тесты для `MissingSections`/`IsClean`. Расхождение — править код, не контракт.
...
- [ ] **STEP 2 (IMPLEMENTATION)**: проверить `launcher.go` — монтирования, `-it` по `isTTY` (`term.IsTerminal`),
  ...
  Зазоров нет; править код при находке.
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/docker` против `.goga/usages/conventions.md`: DI через `ReExecConfig` (явные параметры конструктора), error wrapping (`%w`) при `exec.LookPath`/`os.UserHomeDir`, именованные импорты; `ReExec` намеренно не принимает `context.Context` (`syscall.Exec` не возвращает управление) — задокументированное исключение, не gap. Расхождение — править код, не контракт.
...
- `logger_facade.md` (`pkg/progress/.usages/logger_facade.md`): API `progress.Logger`, потребляемое здесь
  (`NewLogger`/`Log`/`LogAction`/`LogStart`/`LogEnd`/`Close`), консьюмер-сторона того же файла, что и Task 4.
...
- [ ] **STEP 2 (IMPLEMENTATION)**: проверить `executor.go`/`transcript.go`/`runner.go` — `DefaultClaudeArgs` (с `--verbose`),
  ...
  Зазоров нет; править код при находке.
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/executor` против `.goga/usages/conventions.md`: `context.Context` первым параметром в методах `Runner` (`RunAgent`/`RunPlanning`), DI через конструктор (`Runner::Executor(cfg Config)`), `log/slog`-совместимое логирование через `progress.Logger`, bounded concurrency для стриминга stdout/stderr (без неограниченных горутин), именованные импорты. Расхождение — править код, не контракт.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/orchestrator` (errors/buses/graph/context) против `.goga/usages/conventions.md`: `errors.Is`/`errors.As`+`%w` в `Classify`, DI через конструкторы у `UIBus`/`CriticalBus`, bounded concurrency в fan-out подписчиков (без утечки горутин при `SubscriberDroppedCount`), именованные импорты. Расхождение — править код, не контракт.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/orchestrator` (FSM) против `.goga/usages/conventions.md`: DI через конструктор `FSM`, явный `GuardCtx` вместо implicit-state, error handling в `Apply` (не паника при illegal transition — `applied=false`), table-driven/property-тесты (`rapid`) соответствуют table-driven конвенции, именованные импорты. Расхождение — править код, не контракт.
- [ ] **STEP 3 (INTERFACE VERIFICATION)**: `go test ./pkg/orchestrator/...` — зелёные.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/orchestrator` (Orchestrator + dialog poller) против `.goga/usages/conventions.md`: DI через `Options` (явные параметры конструктора, не service locator), `context.Context` пробрасывается в вызовы `Runner`, `log/slog`-логирование, bounded concurrency (`activeAgents sync.Map`, `time.Ticker` с явным жизненным циклом — не unbounded goroutine leak), именованные импорты. Расхождение — править код, не контракт.
- [ ] **STEP 3 (INTERFACE VERIFICATION)**: `go test ./pkg/orchestrator/...` — зелёные.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/server` (server.go) против `.goga/usages/conventions.md`: DI через конструктор `Server`/`Config`, `context.Context` из `*http.Request` пробрасывается далее, `log/slog`-логирование, именованные импорты. Расхождение — править код, не контракт.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `pkg/server` (handleDialogAnswer) против `.goga/usages/conventions.md`: error wrapping (`%w`) при файловых операциях, `context.Context` запроса используется для отмены/таймаутов при I/O, именованные импорты. Расхождение — править код, не контракт.
- [ ] **STEP 3 (INTERFACE VERIFICATION)**: `go test ./pkg/server/...` — зелёные.
...
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `cmd/afm` (entrypoint + Поток A) против `.goga/usages/conventions.md`: DI через явные конструкторы (`orchestrator.New(Options{...})`, `server.New(Config{...})`), `context.Context` пробрасывается из `cobra.Command` (`cmd.Context()`) в `RunE`, `log/slog`-логирование, именованные импорты. Расхождение — править код, не контракт.
...
- [ ] **STEP 2 (IMPLEMENTATION)**: проверить `approve.go`/`check.go`/`retry.go`/`revise.go`/`init.go`/`list.go`/`install_skills.go` —
  ...
  **Constraint:** CLI-мутации `Store.Apply` — осознанное исключение `tools/setstatuslinter` (без живого Orchestrator).
  Зазоров нет; править код при находке.
- [ ] **STEP 2b (CONVENTIONS CHECK)**: проверить `cmd/afm` (CLI-команды) против `.goga/usages/conventions.md`: error wrapping (`%w`) в approve/retry/revise, именованные импорты, DI через явные параметры (не глобальное состояние) в конструкторах команд. Расхождение — править код, не контракт.
- [ ] **STEP 3 (INTERFACE VERIFICATION)**: `go test ./cmd/afm/...` — зелёные.
...
- `go test ./...`: весь тест-suite зелёный (включая уже существующий `pkg/web/embed_test.go`, см. Task 12 — новый тест не создаётся).
...
- [ ] Все 8 сценариев дизайн-дока §Test Stack Trace назначены владеющей клеточке и верифицированы
  ...
  `TestWeb_Embed_ServesOriginalWebPaths_AfterDirMove` — эквивалент `TestEmbed_ServesOriginalWebPathsAfterDirMove`
  уже существует (`pkg/web/embed_test.go`), верифицирован Task 12,
...
- [ ] Все 21 `[verify/conform]`-задачи прошли STEP 2b (CONVENTIONS CHECK): код клеточки сверен с
  `.goga/usages/conventions.md` (именованные импорты, DI через конструктор, `context.Context` первым
  параметром, `errors.Is`/`errors.As`+`%w`, `log/slog`, bounded concurrency, table-driven тесты);
  расхождения исправлены в коде, контракты не изменены.
