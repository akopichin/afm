...
Pass both `addr` and `shimDir` into `executor.Config.ProxyURL`/`ProxyShimDir` (see
`pkg/executor`'s `runner_facade`) so every spawned agent process routes through the proxy — including
wrapper commands that clobber `ANTHROPIC_BASE_URL` themselves, since the shim's `claude` script
re-injects the proxy address via `PATH` precedence.
