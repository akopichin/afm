...
- **The pipeline itself is code, not a single smart agent** (`maybeRunReflection`/`runReflectionPipeline`, `pkg/orchestrator/reflection.go`), triggered from `completeStage` next to the existing `maybeRunAfterHook`, **after** the unblock cascade — deliberately in the background, the same ordering guarantee `script_after` already has, so reflection never blocks downstream stages:
  ...
  All three agent steps go through the single seam `o.runMemoryAgent func(ctx, memoryAgentSpec) error` (defaulted in `New()` to `execMemoryAgent`, `memory_agent.go`) — tests override it (`pipelineHarness`, `reflection_test.go`) to simulate each step's file effects without spawning real processes. Every write to the two `.md` files goes through `atomicWriteFile` (temp+rename) — a killed agent never leaves a half-written memory file.
