...
## Agent memory (reflect → updater → compressor)

Each stage runs an agent in an isolated context — whatever it learns (an API's real behavior, a required build flag, a rule it broke and had to correct) evaporates when the stage finishes. Agent memory is an opt-in, **code-driven** pipeline of three small agents that runs after a stage completes, distills its session into findings, and merges them into two memory files that later stages (and later runs) are told to read themselves. Spec: `docs/superpowers/specs/2026-08-28-agent-memory-design.md`; plan: `docs/superpowers/plans/2026-08-28-agent-memory.md`.

- **Enabling: `memory:` on `flow.Flow` + `reflect: true` per stage.** `flow.MemoryConfig` (`pkg/flow/flow.go`) has `project_file` (path to `PROJECT_MEMORY.md`, relative to `root_dir` — **non-empty is what enables the whole feature**, everything else is inert without it), `max_bytes` (per-file compression threshold, default `20000`), `compress_retries` (compressor attempts before the terminal fallback, default `2`), and `final_reflect` (bool, run one reflection over the whole flow at `Run()` exit). `Flow.MemoryEnabled() bool` is just `Memory.ProjectFile != ""`. Per-stage, `Stage.Reflect bool` (`yaml:"reflect,omitempty"`, default `false`) opts that one stage into feeding memory after it finishes. `Flow.validate()` rejects `reflect: true` (or `final_reflect: true`) while `project_file` is empty (`stage %q: reflect requires memory.project_file`); a `script:` stage with `reflect: true` parses fine but is silently skipped at runtime (`stage.IsScript()` check in `maybeRunReflection`, `reflection.go`) — a script stage has no agent session to reflect on.

- **Two-file scope, different lifetimes.** `PROJECT_MEMORY.md` lives **in the repo** at `memory.project_file` (resolved against `root_dir` in `cmd/afm/run.go`) and accumulates **across runs** — long-term. `SESSION_MEMORY.md` is auto-located at `.afm/runs/<run>/SESSION_MEMORY.md` (not configurable) and is **reset to an empty header stub at the start of every run** by `(*Orchestrator).initSessionMemory()` (called first thing in `Run()`, before `startPlanningForPending`) — a previous run's session notes never leak into a new one. Both files use a "Do's/Don'ts" format (🟩 Best Practices / 🟥 Anti-Patterns), written by the updater agent.

- **The pointer, not the content, is injected into every prompt.** afm does NOT read or inline memory content into prompts — that would make prompt size grow with memory. Instead, `cmd/afm/run.go` appends a short pointer block (built by `buildMemoryPointer(projectPath, sessionPath)`, `cmd/afm/memory_pointer.go`) naming the two **absolute** paths to `globalPrompt` (`Flow.Prompt` + the pointer), which becomes `Options.GlobalPrompt` and is forwarded into every `prompts.Build` call by every stage runner (planning/implementation/review/autonomous) — so it reaches every stage regardless of that stage's own `reflect` value, not just reflecting stages. The agent reads both files itself with its own `Read` tool.

- **The pipeline itself is code, not a single smart agent** (`maybeRunReflection`/`runReflectionPipeline`, `pkg/orchestrator/reflection.go`), triggered from `completeStage` next to the existing `maybeRunAfterHook`, **after** the unblock cascade — deliberately in the background, the same ordering guarantee `script_after` already has, so reflection never blocks downstream stages:
  1. **reflect** — a fresh-context agent (no `--resume`, template `runJSONFixAgent`) reads the stage's `<phase>.log` (+ summary/plan) and writes ONE YAML dataset, `<stageDir>/reflect_dataset.yaml` (`project_level`/`session_level` findings, RL/RLHF shape — a free byproduct kept on disk, not just an intermediate).
  2. **updater** — reads that dataset plus the current `PROJECT_MEMORY.md`/`SESSION_MEMORY.md` and rewrites **both** files in place, deduplicated and consolidated.
  3. **size-check (pure code)** — `fileExceeds(path, maxBytes)` (`memory_size.go`) after the updater, for each of the two files.
  4. **compressor**, up to `compress_retries` times per oversized file, lossily distilling it back under the threshold; on the LAST attempt a dynamic tail is appended to its prompt (`lineLimitForBytes(maxBytes)`) forcing an "extreme compression" pass to under N lines.
  5. **terminal fallback (pure code)** — if still over after all retries, `fifoDropOldestBlocks` (`memory_size.go`) deterministically drops the OLDEST `## …` blocks (FIFO by position, NOT true LRU — afm has no recency signal) until the file fits, and a `reflect_failed`-shaped warning notice is published. The header (`# …`) is never dropped; the whole file is never destroyed.
  All three agent steps go through the single seam `o.runMemoryAgent func(ctx, memoryAgentSpec) error` (defaulted in `New()` to `execMemoryAgent`, `memory_agent.go`) — tests override it (`pipelineHarness`, `reflection_test.go`) to simulate each step's file effects without spawning real processes. afm's own direct file touches — `initSessionMemory` (session stub) and `fifoDropOldestBlocks` (FIFO fallback) — go through `atomicWriteFile` (temp+rename); the updater and compressor **agents** rewrite `PROJECT_MEMORY.md`/`SESSION_MEMORY.md` in place with their own Write tool, so a killed agent can leave a partially written memory file (acceptable: memory is best-effort and outside crash-recovery — the next run's updater re-consolidates).
  Agent isolation mirrors `runJSONFixAgent`: fresh session, no `AFM_STAGE_DIR` (these agents aren't dialog participants, they read/write absolute paths given directly in the prompt), separate log files (`reflect.log`/`updater.log`/`compressor-<file>.log` under the stage dir) so their tool actions never land in the stage's own `<phase>.jsonl`.

- **Serialization + shutdown bookkeeping.** A single `o.reflectMu` (size-1 slot, mirrors an exclusive lock, not a semaphore) is held for the FULL duration of one pipeline run, so with `max_parallel > 1` two stages' pipelines never interleave writes to the shared `.md` files — they queue instead (acceptable: reflection is background/best-effort, queueing latency doesn't matter). `o.pendingReflections atomic.Int32` mirrors the existing `pendingAfterHooks` counter exactly: incremented synchronously in `maybeRunReflection` before `concurrency.SpawnDetached`, decremented on completion, and both `shouldExit()`/`allTerminal()` (`scheduling.go`) account for it — the run cannot report done while a pipeline is still in flight. `SpawnDetached` (not `SpawnAgent`) is used because these agents must NOT take the `max_parallel` command semaphore or overwrite the stage's own active-agent marker; they're still tracked in `agentWG` for clean shutdown, bounded by the same 10s `waitAgents()` window as everything else.

- **Best-effort, never touches the FSM.** Any step failing aborts the rest of the pipeline for that stage but leaves memory in a consistent state (atomic writes mean no partial file); a `reflect_failed` notice is surfaced both live (`o.ui.Publish`) and durably via `stagefiles.AppendNotice` into `notices.jsonl`, exactly like `runAfterHook`'s failure path — the stage's own FSM/status is completely unaffected either way. There is no crash recovery: an afm crash mid-pipeline loses that stage's reflection in v1 (no FSM transition, nothing in `events.jsonl` to replay from).

- **`final_reflect` — one more pass over the whole flow at `Run()` exit.** When `memory.final_reflect && project_file != ""`, `(*Orchestrator).runFinalReflectionOnce(ctx)` runs the SAME reflect→updater→size-check→compressor pipeline once, guarded by `o.finalReflectDone` (idempotent — a second call is a no-op), with the reflect agent pointed at `RunDir` itself (so it reads `<run>/*/<phase>.log` across all stages, not just one). It's called from `Run()`'s event loop exactly at the point `shouldExit()` first becomes true, **before** `return nil` — synchronously, so `Run()` doesn't report the flow done until it finishes. By that point `pendingReflections == 0` is already part of `shouldExit()`, so every per-stage pipeline has already drained and the final pass sees a fully consistent, complete-run state.

- **Three overridable embedded prompts.** `orchestrator.Prompts` carries `Reflect`/`Updater`/`Compressor` alongside the existing `Planning`/`Implementation`/`Review`/`Summary`/`Autonomous`; `cmd/afm/run.go`'s `loadPrompts` loads `reflect.md`/`updater.md`/`compressor.md` the same way as the others — embedded defaults under `assets/prompts/`, overridable per-project via the existing `prompts_dir` config, no new override mechanism. `buildMemoryPrompt(Prompts, memoryAgentSpec)` (`memory_prompts.go`) wraps the loaded template with the concrete absolute file-I/O instructions (which files to read, the exact path to write) for that specific pipeline step.
