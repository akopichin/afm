...
## Agent memory (structured store, afm-owned) — v2

Each stage runs an agent in an isolated context — whatever it learns (an API's real behavior, a required build flag, a rule it broke and had to correct) evaporates when the stage finishes. Agent memory is an opt-in pipeline that runs after a stage completes, distills its session into **structured findings**, and merges them into two YAML stores that later stages are shown a **relevant slice** of, not the whole thing. v2 replaces v1's prose `.md` files + three-agent pipeline with a `pkg/memory`-owned structured store + a two-agent pipeline (**reflect → consolidator**; the compressor is gone) — afm (code), not an agent's Write tool, owns validation, metadata, eviction, and atomic writes. Supersedes the v1 design. Spec: `docs/superpowers/specs/2026-08-28-agent-memory-v2-design.md` (see also the superseded v1 spec/plan for what's unchanged in the enablement/scheduling layer, restated below).

- **Enabling — unchanged from v1.** `memory:` on `flow.Flow` (`flow.MemoryConfig`, `pkg/flow/flow.go`) with `project_file` (now a `.yaml` path, relative to `root_dir` — **non-empty is what enables the whole feature**) plus per-stage `Stage.Reflect bool` (`yaml:"reflect,omitempty"`, default `false`). `Flow.validate()` still rejects `reflect`/`final_reflect: true` without `project_file`; `script:` stages are still silently skipped (`stage.IsScript()` in `maybeRunReflection`, `reflection.go`) — no agent session to reflect on.

- **The store: `pkg/memory.Finding`/`Store` — a structured YAML type, not prose.** `Finding` (`pkg/memory/memory.go`) carries `ID`/`Scope` (`project`|`session`)/`Kind` (`fact`|`best_practice`|`anti_pattern`)/`Topic` (tags for retrieval)/`Statement`/`Evidence` (REQUIRED — `Finding.Valid()` rejects empty evidence, unknown scope/kind, or a malformed `ID`)/`FirstSeen`/`LastSeen` (run-ids, not wall-clock)/`ConfirmCount`/`SourceStage`. `Store{Findings []Finding}` is the whole file, `memory.Load`/`memory.Save` (`pkg/memory/io.go`) read/write it — `Save` is atomic (temp+rename), matching v1's `atomicWriteFile` discipline but now the *only* writer of the store files (agents never touch them, see below). **afm owns all metadata** — agents never set `id`/`first_seen`/`last_seen`/`confirm_count`.

- **Two-file scope, same lifetimes as v1.** `PROJECT_MEMORY.yaml` lives **in the repo** at `memory.project_file` (resolved against `root_dir` in `cmd/afm/run.go`) and accumulates **across runs**. `SESSION_MEMORY.yaml` is auto-located at `.afm/runs/<run>/SESSION_MEMORY.yaml` and **reset to an empty `Store` at the start of every run** by `(*Orchestrator).initSessionMemory()` (`memory.Save(path, memory.Store{})`, unchanged trigger point). Only the file *format* changed, `.md` → `.yaml`; a pre-existing v1 `.md` file is simply not read (no migration — the structured store starts empty).

- **The pipeline is TWO agents, not three** (`maybeRunReflection`/`runReflectionPipeline`, `pkg/orchestrator/reflection.go`), triggered from `completeStage` exactly like v1 — background, best-effort, after the unblock cascade:
  1. **reflect** (`memoryKindReflect`) — fresh-context agent, reads the stage's `<phase>.log`(+ summary/plan) and writes candidate findings as a `Store` YAML to `<stageDir>/reflect_dataset.yaml`. The prompt (`assets/prompts/reflect.md`, rewritten for v2) explicitly **excludes afm/agent-protocol mechanics** (execution_summary format, `$AFM_STAGE_DIR`, dialog naming, approval/retry flow — the P1 signal-quality problem v1 had) and **requires evidence** on every finding; nothing durable and evidenced → `findings: []`.
  2. **consolidator** (`memoryKindConsolidator`) — fresh-context agent, reads the candidate dataset plus BOTH current stores (`projectPath`/`sessionPath` in `memoryAgentSpec`) and returns a `memory.MergedStore` (`pkg/memory/reconcile.go`): each finding tagged `status: new|reinforced|unchanged`. It semantically merges/generalizes duplicates (the v1 updater's job, kept) AND acts as the verification gate v1's missing "third agent" would have been — it drops candidates that aren't durable/project-specific/evidenced, so afm doesn't need a separate verify pass. **The compressor agent is REMOVED** — no size-triggered LLM rewrite exists in v2 at all.
  3. **afm (code) reconciles + writes** — `reconcileAndSave` (`reflection.go`) reads the consolidator's `consolidated.yaml`, calls `memory.Reconcile(prevStore, merged, runID)` (`pkg/memory/reconcile.go`): `new` → `first_seen=last_seen=runID`, `confirm_count=1`, afm assigns a stable slug `id` if missing/colliding (`slugFromStatement`); `reinforced` → `last_seen=runID`, `confirm_count++` (looked up from the previous store by `id`); `unchanged` → keeps the previous finding verbatim. Invalid findings (`!Finding.Valid()`) are silently dropped — never abort the pipeline. The result is split by `Scope`, evicted, and `memory.Save`d per scope.
  4. **eviction (code, `pkg/memory/evict.go`)** — `memory.Evict(store, maxFindings)` replaces v1's FIFO drop: keeps the `maxFindings` highest-value findings, ordered by `ConfirmCount` desc then `LastSeen` desc (ties broken by original index) — deterministic, no LLM. `reflect_dataset.yaml` and `consolidated.yaml` stay on disk under the stage dir as pipeline byproducts (auditable, same as v1's dataset file).
  All agent steps go through the same seam `o.runMemoryAgent func(ctx, memoryAgentSpec) error` (defaulted to `execMemoryAgent`, `memory_agent.go`) — tests override it (`pipelineHarness` in `reflection_test.go`, and the harness in `memory_integration_test.go`) to simulate file effects without spawning real processes. Agent isolation is unchanged from v1: fresh session (no `--resume`), no `AFM_STAGE_DIR`, separate `reflect.log`/`consolidator.log` under the stage dir.

- **Retrieval into the prompt (P2) — a per-stage slice, not a static pointer.** v1 injected the SAME pointer block into every stage's `GlobalPrompt` regardless of relevance. v2's `(*Orchestrator).memoryBlockForStage(s flow.Stage) string` (`pkg/orchestrator/memory_inject.go`) is recomputed **per stage, per `prompts.Build` call** (wired as `prompts.Inputs.MemoryBlock` in every `runXxxAgent`/`*WithFeedback` in `agents.go`):
  - loads both stores (`memory.Load`); `""` if memory disabled or both stores are empty (nothing to point at yet);
  - tokenizes the stage's `ID+Name+Description` (`memory.Tokenize`) and calls `memory.Select(proj, sess, tokens, memory.RetrievalConfig{Threshold, CoreConfirmCount})` (`pkg/memory/retrieval.go`);
  - **small store** (total findings ≤ `retrieval_threshold`) → `Select` signals "inject all"; `memoryBlockForStage` returns the same wording as v1's pointer (`memoryPointerBlock`, moved here from the deleted `cmd/afm.buildMemoryPointer`) naming both absolute paths — the agent reads the files itself;
  - **large store** (> `retrieval_threshold`) → `Select` returns only the **core** (every finding with `confirm_count ≥ core_confirm_count`) plus **relevant** (finding `topic`/`statement` tokens intersecting the stage's tokens) findings; `memory.Render` (`pkg/memory/retrieval.go`) formats them as a compact text block, code-built (no agent, no per-stage file), with a trailing pointer line to the full project file for anything filtered out.
  - Session findings are always small (per-run) and inlined whole when the store is large; both scopes contribute to `Select`.

- **Serialization + shutdown bookkeeping — unchanged from v1.** `o.reflectMu` still serializes one pipeline run at a time across the shared store files; `o.pendingReflections atomic.Int32` still gates `shouldExit()`; `SpawnDetached` still keeps these agents off the `max_parallel` semaphore and out of the active-agent marker, tracked in `agentWG` for clean shutdown.

- **Best-effort, never touches the FSM — unchanged from v1.** Any step failing aborts the rest of the pipeline for that stage; a `reflect_failed` notice is surfaced live + durably (`stagefiles.AppendNotice`); the stage's own FSM/status is untouched either way. Still no crash recovery mid-pipeline.

- **`final_reflect` — unchanged trigger, same two-agent pipeline.** `(*Orchestrator).runFinalReflectionOnce(ctx)` runs reflect→consolidator→reconcile once over the whole run (`RunDir` as source), guarded by `o.finalReflectDone`, called from `Run()`'s event loop right before it reports done.

- **Two overridable embedded prompts, not three.** `orchestrator.Prompts` carries `Reflect`/`Consolidator` (`Updater`/`Compressor` removed, a net −1 embedded prompt vs v1). `cmd/afm/run.go`'s `loadPrompts` loads `reflect.md`/`consolidator.md`; `updater.md`/`compressor.md` are deleted. `buildMemoryPrompt(Prompts, memoryAgentSpec)` (`memory_prompts.go`) has a `reflect`/`consolidator` switch (`memoryKindReflect`/`memoryKindConsolidator` constants) wrapping the loaded template with the concrete absolute file-I/O instructions for that step; `updater`/`compressor` kinds are gone.

- **Config knobs (`flow.MemoryConfig`, `pkg/flow/flow.go`), defaulted in `ParseFile` when memory is enabled:** `max_findings` (per-scope eviction ceiling, default `60`), `retrieval_threshold` (≤ → inject-all/pointer, > → inlined relevant slice, default `25`), `core_confirm_count` (confirm_count at/above which a finding is always inlined, default `3`), `final_reflect` (unchanged, default `false`). **Removed:** `max_bytes`, `compress_retries` — meaningless once the compressor is gone.
