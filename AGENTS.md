...
- **`docker.file_browser.enabled` (`*bool`, default `false`)** — the browser is OFF unless explicitly turned on. `IsEnabled()` resolves in priority order **env `AFM_FILE_BROWSER` > config > default `false`**: `AFM_FILE_BROWSER=1/true` force-enables and `=0/false` force-disables regardless of config (empty/unset → config decides); config `enabled: true` enables, `nil`/`false` keep it off. `IsEnabled()` is read only host-side in `cmd/afm/run.go` (before re-exec), so the env decision governs whether the file-root manifest is forwarded into the container at all — no need to forward `AFM_FILE_BROWSER` itself. `mergeFile` (`pkg/config/config.go`) must copy `overlay.Docker.FileBrowser.Enabled` like every other `Docker.*` field, or a project-layer `file_browser: {enabled: true}` is silently dropped on merge (found by review; regression test `TestLoadFrom_FileBrowserEnabledMergesAcrossLayers`). Env-priority regression: `TestFileBrowser_EnvOverridesConfig`.
...
- **Shell composition (`app/App.tsx`)**: `GlobalHeader` (brand + flow name/desc left, `RunMetrics` — Started/Elapsed/Idle/Backoff, tabular-nums — center, Files/theme/notifications/connection right) → `ReviewBanner` → `<main>` `DashboardShell` (grid: `StagesList` rail ~240px + `.workspace`). Footer removed; progress (done/total) moved into the rail head (`StagesList` `progressDone`/`progressTotal`).
- **Workspace tabs (`components/workspace/WorkspaceTabs.tsx`)**: a permanent **Feed** tab + one contextual **attention/detail** tab. `App` drives it with a simple `activeTab: 'feed' | 'detail'` state (NOT the reducer — see below) plus `attentionKindForStatus`: the detail tab is labeled by attention kind (Approval/Question/Paused/Failed/Hook failed) with a `· N` count and a glow when the selected stage needs action; otherwise it shows the stage name. Auto-switches to detail **once per arrival** (`lastAttentionKey` ref) and NOT while an editable element is focused (`isEditableFocused`, suppression). `WorkspaceHeader` shows the stage context + an `attentionNav` prev/next (`N / M`) when >1 stage awaits (queue from `deriveAttentionItems`).
- **`hooks/use-workspace-view/`** is a pure, fully-tested attention reducer (`feed`/`attention`/`plan-history`/`dialog-history`, auto-open-once, resolve-advance). It is currently used only for its pure helpers (`deriveAttentionItems`, `attentionKindForStatus`, `countByKind`); the full reducer state machine is wired in App by the simple tab state above (kept for a future multi-queue upgrade).
- **Feed (`components/feed-workspace/`)**: `FeedWorkspace` renders a messenger view of the REAL event log (no fabricated text — plan §6). `feed-view-model.ts` maps each event → `FeedItem` (actor `agent`/`user`/`system` → side left/right, tone, kind); `agent_action tool="text"` renders as agent prose, other tools as compact mono rows; `groupFeedItems` merges consecutive same-side+stage items into bubbles. Feed/Log segmented control keeps `useFeedMode` (`afm-feed-mode`), independent stick-to-bottom, Jump to latest. `base/feed-workspace.css` replaced `base/event-feed.css`.
- **Approval/Question** are the existing `PlanPanel`/`DialogChannel` (logic unchanged) rendered in the workspace; `base/*.css` scopes them under `.workspace-body` into wide documents with a fixed/sticky bottom action bar and the redundant `PanelFrame` title hidden. The custom dialog answer now sets `allowFileReferences` (paperclip → project-file references; Docker-only capability). Options render full-width, normal case.
- **Verified live in Docker** (real graphite image, real browser): header/metrics, rail timeline, Feed messenger (agent-left/user-right/success cards), Approval (wide + bottom bar) with attention nav `1/2`, Question (wide + sticky bar + prose options), dark + light, full run lifecycle to done, zero console errors.
