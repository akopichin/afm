...
- **The write chain is FOUR steps — three LLM agents plus one code step — not two agents.** Triggered from `completeStage` exactly like v2 — background, best-effort, after the unblock cascade. `(*Orchestrator).maybeRunReflection` (`pkg/orchestrator/reflection.go`) gates on `MemoryDir != "" && stage.Reflect != nil && stage.Reflect.CanWrite() && !stage.IsScript()`, then:
  1. **reflect** (`memoryKindReflect`, prompt `assets/prompts/reflect.md`) — fresh-context agent, reads the stage's session (`<phase>.log`+summary/plan) and writes an RL-style dataset to `<stageDir>/reflect_dataset.yaml`: one YAML doc with `project_level`/`session_level`, each item `{prompt, chosen, rejected}` in block-literal style. **P1 kept from v2:** a hard EXCLUDE list in the prompt keeps `execution_summary.md` format, `$AFM_STAGE_DIR`, dialog/question naming, plan-approval/autonomous/retry/backoff mechanics, and "read the memory files" itself OUT of the dataset — memory stays project-specific, not framework noise.
  ...
  This shared chain lives in `(*Orchestrator).distill(ctx, stageName, datasets, logDir, targetFile)` (`reflection.go`), serialized by `o.reflectMu` (one pipeline writing shared files at a time) — used both per-stage (`targetFile` = the stage's own `Reflect.File`) and end-of-run (`targetFile` = `memory.md`, see below). All steps go through the seam `o.runMemoryAgent func(ctx, memoryAgentSpec) error` (defaulted to `execMemoryAgent`); tests override it via `stubMemoryAgentByKind` (`reflection_test.go`) keyed by `spec.kind`. Agent isolation unchanged from v2: fresh session (no `--resume`), no `AFM_STAGE_DIR`, separate `reflect.log`/`aggregate.log`/`prioritize.log`/`update.log` per step under the stage dir (end-of-run: under the run dir). `reflect_dataset.yaml`/`patterns.md`/`prioritized.md`/`high.md` stay on disk as auditable byproducts.

- **End-of-run pass: same chain, steps 2-5 only, over ALL stages' datasets.** `(*Orchestrator).runEndOfRunMemory(ctx)` (renamed from v2's `runFinalReflectionOnce`, same trigger point in `Run()`'s event loop, guarded by `o.finalReflectDone`) globs `<RunDir>/*/reflect_dataset.yaml` (every stage's reflect output already written during the run — step 1/reflect is NOT re-run), no-op if none found, otherwise calls `distill` once over all of them into `memory.ProjectFile(MemoryDir)` (`<MemoryDir>/memory.md`). If `memory.commit == true`, `memory.Commit(MemoryDir, "chore(memory): update project memory")` (`pkg/memory/commit.go`) runs afterward — best-effort, notice-only on failure.

- **Injection (read side) — pointer-only, by mode, no relevance slicing.** v2's per-stage relevance retrieval (`Select`/`Render`, `retrieval_threshold`, `core_confirm_count`) is GONE — v3 files are small Markdown, always injected whole as a pointer. `(*Orchestrator).memoryBlockForStage(s flow.Stage) string` (`pkg/orchestrator/memory_inject.go`, wired as `prompts.Inputs.MemoryBlock` in every `runXxxAgent`/`*WithFeedback` in `agents.go`, recomputed per stage per `prompts.Build` call): names `memory.ProjectFile(MemoryDir)` for **every** stage if it exists on disk, PLUS `memory.StageFile(MemoryDir, s.Reflect.File)` for **that** stage only if `s.Reflect.CanRead()` (mode `r`/`rw`) and that file exists. `mode: w` → the stage's file is written but never injected back into that same stage. `""` when memory disabled or nothing exists yet on disk (not even the project file). The agent reads the named absolute path(s) itself — no inlining.
