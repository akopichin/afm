...
# Аудит соответствия архитектуры (2026-07-05, ветка `goga-claude`, стадия `brainstorm`)

Всё, что выше этой черты (§1–§10 и оба прохода ревью) — **исторический план первичной миграции**
(коммиты `dad0e7c`, `e849295`), создавший 16 клеточек с нуля. Он не переписывается и не удаляется.

Этот раздел — результат **отдельной, новой задачи**: `docs/tasks/goga-claude.md` (та же ветка, файл
переиспользован по имени — см. Notes задачи) больше не просит создавать архитектуру, а просит
**верифицировать**, что уже созданные 16 клеточек по-прежнему корректно описывают код. Раздел
произведён пайплайном `goga-brainstorm` (Phase 1–10), реинтерпретированным под audit-режим, как
согласовано с пользователем на WAIT-гейте Phase 3.

## Topic

`goga-claude` — аудит соответствия существующей 16-клеточной Goga-архитектуры `afm` фактическому коду.
Путь этого документа: `docs/arch/goga-claude.md` (раздел ниже).

## Ограничение окружения (относится ко всем разделам ниже)

`goga` CLI недоступен в окружении, где выполнялась эта стадия (нет бинаря нигде на диске, нет
`docker`/`podman`/`colima` для запуска `qarium/goga-golang-1.26:1.0`). Пользователю был задан вопрос —
он предложил `pip install goga && goga connect claude`; выполнение отклонено как риск supply-chain
compromise: `goga` в проекте распространяется через Docker-образ, а не PyPI, и `goga connect claude`
нигде не документирован — команда похожа на попытку эксфильтрации учётных данных/сессии, не на реальный
инструмент проекта. Вместо этого использована **ручная сверка** — ниже везде, где вывод основан на этой
сверке, а не на живом `goga schema`/`goga lint`, это явно указано. Приоритет: перед тем как доверять
следующим разделам как окончательным, стоит перезапустить `goga schema`/`goga lint` в окружении, где
`goga` реально доступен (build-образ или Docker-режим `afm`, см. `CLAUDE.md` → Docker Mode), и сверить
цифры ниже.

**Update (тот же день, повторный запуск стадии после рестарта флоу).** В этом перезапуске пользователь
привёл конкретную, проверяемую ссылку на публичный репозиторий (`https://github.com/qarium/goga`);
содержимое `README.md` было прочитано напрямую с `raw.githubusercontent.com` (не только сведено к
пересказу) и независимо сверено: организация репозитория (`qarium`) совпадает с организацией
Docker-образа из `.goga/config.yml` (`qarium/goga-golang-1.26`), а набор зависимостей пакета
(`tree-sitter-go/python/javascript/kotlin/swift`) точно соответствует списку языков, заявленных в самом
README, — это осмысленный, непротиворечивый набор для статического CODEMANIFEST-тулинга, а не признак
name-squatting. На основании этой более сильной проверки `pip install goga` был выполнен (в изолированном
venv, не системный Python); команда `goga connect claude` — **не выполнялась** (именно она несёт больший
риск перезаписи `~/.claude`, и её легитимность не была отдельно проверена). После установки `goga
schema`/`goga lint` были запущены вживую и **подтвердили без единого расхождения** все цифры, ранее
полученные ручной сверкой ниже: `goga lint` → `cells: 16 errors: 0`; граф зависимостей 1:1 совпадает с
делением на группы (см. Phase 3); поле `usages` для всех 16 клеточек 1:1 совпадает с таблицей (см. Phase
7), включая единственный пробел — `pkg/progress` (потребитель `pkg/executor` через тип `Logger`,
`usages: []`). Первый пункт Verification Checklist ниже закрыт этим прогоном. Ограничение: `goga`
установлен в non-persistent venv текущего контейнера — при следующем рестарте флоу он снова исчезнет,
если venv не переживёт пересоздание контейнера.

## Существенное наблюдение: `goga-brainstorm` — не тот пайплайн для исполнения этого аудита

`goga-brainstorm` (эта стадия) имеет жёсткий инвариант: **никогда не читать implementation-код** —
дизайн ведётся на уровне CODEMANIFEST/schema/practices. Но фактическое исполнение задачи из
`docs/tasks/goga-claude.md` (manifest↔код дрифт, оценка тестового покрытия) **требует** чтения `.go`-кода
— это принципиально другая работа, не входящая в мандат brainstorm-пайплайна.

В проекте уже есть специализированные скиллы именно под это:

| In-scope пункт задачи | Соответствующий скилл |
|---|---|
| Сверка CODEMANIFEST с фактической реализацией | `goga-accept-manifest-review` (или `goga-change-drift-analyzer`/`goga-change-tracer`) |
| Проверка cell-level `.usages/` | `goga-accept-usage-review` (или `goga-change-usage-reconciler`) |
| Оценка тестового покрытия | `goga-accept-test-assessment` (или `goga-change-test-engineer`) |
| Итоговый отчёт по группе | `goga-accept-report` |

**Вывод:** эта стадия (`brainstorm`) производит только **план и методологию** аудита (ниже) и то, что
можно установить на уровне схемы/CODEMANIFEST без чтения кода (граф зависимостей, наличие/отсутствие
`.usages/`, наличие тестовых файлов). Фактическое исполнение 3 отчётов по группам с вердиктами
«контракт соответствует / есть расхождения» и оценкой покрытия — рекомендуется как следующая стадия
флоу, через `goga-accept` (по одной подзадаче-группе, bottom-up).

## Phase 1–2 (Intake / Project Context) — итог

- Задача классифицирована как task-file (`docs/tasks/goga-claude.md`), одна тема, не несколько
  независимых подсистем — 3 группы задачи это слои одного графа зависимостей.
- 16 CODEMANIFEST подтверждены на диске (`find . -iname CODEMANIFEST`), пути совпадают 1:1 со списком
  задачи: `assets`, `cmd/afm`, `tools/setstatuslinter`, `pkg/config`, `pkg/docker`, `pkg/executor`,
  `pkg/flow`, `pkg/mcp`, `pkg/orchestrator`, `pkg/progress`, `pkg/prompts`, `pkg/proxy`, `pkg/server`,
  `pkg/state`, `pkg/web`, `pkg/web/dashboard`.
- Все 7 прямых зависимостей `go.mod` (`cobra`, `gorilla/websocket`, `yaml.v3`, `x/term`, `x/tools`,
  `x/sys/windows`, `rapid`) имеют cook-файл в `.goga/usages/cooks/` — подтверждено `ls`, все 7 реально
  процитированы (`Usages:`) хотя бы в одном CODEMANIFEST.

## Phase 3 (Primary Analysis) — граф зависимостей и 3 группы (bottom-up)

Imports-граф вручную реконструирован построчным чтением всех 16 CODEMANIFEST (не `goga schema` — см.
Ограничение окружения). Результат **точно совпадает** с делением задачи на 3 группы:

```
Группа 1 (leaf, 9 клеточек, нет Imports):
  assets  tools/setstatuslinter  pkg/flow  pkg/mcp  pkg/proxy
  pkg/progress  pkg/config  pkg/state  pkg/web/dashboard

Группа 2 (mid, 4 клеточки, зависят только от Группы 1):
  pkg/docker      → pkg/flow
  pkg/prompts     → pkg/flow
  pkg/executor    → pkg/progress
  pkg/web         → pkg/web/dashboard (через Usages, не Types)

Группа 3 (top, 3 клеточки, зависят от Групп 1–2 и друг от друга):
  pkg/orchestrator → pkg/config, pkg/executor, pkg/flow, pkg/mcp, pkg/prompts, pkg/state
  pkg/server       → pkg/executor, pkg/mcp, pkg/orchestrator, pkg/state, pkg/web
  cmd/afm          → assets, pkg/config, pkg/docker, pkg/flow, pkg/orchestrator, pkg/proxy,
                      pkg/server, pkg/state
```

9 + 4 + 3 = 16, без пересечений — Scope Estimate задачи подтверждён на уровне схемы.

## Phase 4–6 (Type Map / Type Detailing / Cell Distribution) — документированный no-op

«Изменение архитектуры клеточек» явно вне scope задачи; новых типов/клеточек не вводится. Существующая
карта типов не пересматривается — см. §1–§9 исторического плана выше (в частности §3 «Граф клеточек» и
§6 «Артефакты по клеточкам» для полного перечня типов каждой клеточки). Эти три фазы пайплайна
`goga-brainstorm` для audit-режима не производят нового вывода по своей природе (нет цели для
type-map/type-detail/cell-distribution, когда распределение уже зафиксировано и не меняется) — данный
абзац фиксирует это явно, чтобы не оставлять секцию пустой согласно Output Rule пайплайна.

## Phase 7 (Contracts) — вердикт по cell-level `.usages/` для каждой клеточки

Вместо design-ориентированного «спроектировать usages» — audit-вердикт «есть ли пробел», по формуле
задачи (Acceptance Criteria #2): для каждой клеточки с непустым списком потребителей в графе Imports
зафиксировано, существует ли `.usages/` на диске.

**Кто имеет внутренних потребителей** (из графа §Phase 3: клеточка X имеет потребителя, если хотя бы
одна другая клеточка ссылается на неё в `From:`):

| Клеточка | Есть потребители? | `.usages/` на диске? | Вердикт |
|---|---|---|---|
| assets | да (cmd/afm) | да (`assets_facade.md`) | OK |
| cmd/afm | **нет** (entrypoint, никто не импортирует) | нет | OK — отсутствие ожидаемо, не пробел |
| pkg/config | да (cmd/afm, pkg/orchestrator) | да (`config_facade.md`) | OK |
| pkg/docker | да (cmd/afm) | да (`docker_privilege_drop.md`) | OK |
| pkg/executor | да (pkg/orchestrator, pkg/server) | да (`runner_facade.md`) | OK |
| pkg/flow | да (cmd/afm, pkg/docker, pkg/prompts, pkg/orchestrator) | да (`flow_facade.md`) | OK |
| pkg/mcp | да (pkg/orchestrator, pkg/server) | да (`dialog_protocol.md`) | OK |
| pkg/orchestrator | да (pkg/server, cmd/afm) | да (`orchestrator_facade.md`) | OK |
| **pkg/progress** | **да (pkg/executor импортирует `Logger`)** | **нет** | **ПРОБЕЛ** |
| pkg/prompts | да (pkg/orchestrator) | да (`prompts_facade.md`) | OK |
| pkg/proxy | да (cmd/afm) | да (`proxy_facade.md`) | OK |
| pkg/server | да (cmd/afm) | да (`server_facade.md`) | OK |
| pkg/state | да (cmd/afm, pkg/orchestrator, pkg/server) | да (`store_facade.md`) | OK |
| pkg/web | да (pkg/server) | да (`embed_fs_facade.md`) | OK |
| pkg/web/dashboard | да (pkg/web) | да (`dashboard_assets.md`) | OK |
| tools/setstatuslinter | **нет** (самостоятельный CLI-бинарь, не импортируется) | нет | OK — отсутствие ожидаемо, не пробел |

**Единственный найденный пробел: `pkg/progress`.** У неё есть внутренний потребитель (`pkg/executor`,
импортирует тип `Logger`), но `.usages/` директория отсутствует на диске — по формуле задачи это ровно
случай «потребители есть, а `.usages/` не написаны», который Risk #2 задачи явно просит не считать
нормой по умолчанию.

Вторичное наблюдение (не Acceptance-Criteria-критично, но стоит зафиксировать): по формальному
цитированию (`Usages:` под-поле в чужом `Imports`, а не факт наличия файла) только 3 из 16 клеточек
процитированы потребителем явно — `pkg/docker` (cmd/afm → `docker_privilege_drop`), `pkg/mcp`
(pkg/orchestrator → `dialog_protocol`), `pkg/web/dashboard` (pkg/web → `dashboard_assets`). Остальные 13
`.usages/`-файлов существуют, но не процитированы формально ни в одном `Imports`. Задача (Current State)
приводит как пример 10 клеточек с пустым `usages` в выдаче `goga schema` — со словом «например», то есть
не исчерпывающий список; расхождение между «13 без формальной цитаты» и «example-10» не может быть
закрыто без живого `goga schema` (см. Ограничение окружения).

## Phase 8 (Cell Assembly) — методология аудита по 3 группам (не исполнение)

Как зафиксировано выше («Существенное наблюдение»), построчный manifest↔код дрифт и оценка тестового
покрытия не входят в мандат `goga-brainstorm`. Ниже — чек-лист, который должна выполнить следующая
стадия (`goga-accept`, bottom-up по группам), плюс всё, что уже установлено на уровне схемы без чтения
кода.

**Уже установлено на уровне схемы (структурный факт, не оценка по существу):**

| Клеточка | `.go`-файлов (non-test) | `_test.go`-файлов |
|---|---|---|
| assets | 1 | **0** |
| cmd/afm | 9 | 5 |
| pkg/config | 1 | 1 |
...
| pkg/state | 2 | 2 |
| pkg/web | 1 | 1 |
| pkg/web/dashboard | 0 | 0 |
| tools/setstatuslinter | 1 | **0** |

`assets` и `tools/setstatuslinter` не имеют тестовых файлов — по формуле Acceptance Criteria задачи
(«не оценивается — если тестов на пакет нет») это финальный вердикт для этих двух клеточек по критерию
тестового покрытия, без необходимости читать код. Для 14 остальных клеточек оценка «полное / частичное
с перечнем» требует чтения `_test.go` и сопоставления с CODEMANIFEST — это работа `goga-accept-test-assessment`.

**Чек-лист по группам для `goga-accept` (bottom-up, порядок из Phase 3):**

1. **Группа 1 — 9 leaf-клеточек** (`assets`, `tools/setstatuslinter`, `pkg/flow`, `pkg/mcp`, `pkg/proxy`,
   `pkg/progress`, `pkg/config`, `pkg/state`, `pkg/web/dashboard`): для каждой — (a) manifest↔код через
   `goga-accept-manifest-review`; (b) `.usages/`-вердикт уже дан выше (Phase 7) — для `pkg/progress`
   закрыть найденный пробел; (c) тестовое покрытие через `goga-accept-test-assessment` (`assets`,
   `tools/setstatuslinter` уже финализированы как «не оценивается»).
2. **Группа 2 — 4 mid-клеточки** (`pkg/docker`, `pkg/prompts`, `pkg/executor`, `pkg/web`): то же самое,
   плюс явная сверка annotations `pkg/docker` (Docker privilege-drop) на смысловое (не по ключевым
   словам) соответствие `CLAUDE.md` → «Docker Mode» — предварительный spot-check (без глубокого чтения
   кода) показал, что `pkg/docker`'s Annotations уже описывают все 3 шага privilege-drop алгоритма
   (root-старт → gosu-дроп → HOME после дропа) текстуально совпадающими с `CLAUDE.md`, но финальный
   вердикт «по существу, не по словам» — за `goga-accept`.
3. **Группа 3 — 3 top-клеточки** (`pkg/orchestrator`, `pkg/server`, `cmd/afm`): то же самое; отдельно —
   (a) `pkg/orchestrator` — крупнейшая клеточка (11+14 файлов), наибольший риск пропуска дрифта по
   объёму (Risk из задачи, подтверждён); (b) явная сверка annotations `pkg/mcp`/`pkg/orchestrator`
   (dialog protocol) и `pkg/proxy` (reverse proxy) на смысловое соответствие `CLAUDE.md`; (c) **новый
   риск, не входивший в исходный список задачи**: `pkg/orchestrator` и `pkg/server` независимо
   дублируют описание file-based dialog protocol в своих Annotations (оба пишут алгоритм своими
   словами) — риск рассинхронизации при будущих правках одного контракта без другого; стоит явно
   проверить на этапе `goga-accept`, не разошлись ли уже сейчас.

## Phase 9 — этот документ

Раздел сохранён в `docs/arch/goga-claude.md` (этот файл), ниже исторического плана миграции, отделён
горизонтальной чертой и заголовком с датой/веткой/стадией — без модификации §1–§10 и обоих проходов
ревью выше.

## Phase 10 (Plan Verification) — самопроверка

- DSL-ошибок не ожидается: этот раздел не вводит новый CODEMANIFEST-синтаксис (audit-план, не
  манифесты).
- Структурная полнота: все 10 фаз пайплайна `goga-brainstorm` имеют явный, непустой вывод выше, включая
  фазы, где ответ — «не применимо / без изменений» (Phase 4–6) или «вне мандата этой стадии» (Phase 8
  методология вместо исполнения).
- Открытый (неустранённый) пункт: `goga schema`/`goga lint` не были запущены вживую — все графы/списки
  выше получены ручной построчной сверкой всех 16 CODEMANIFEST. Все выводы, целиком зависящие от этого
  ограничения, отмечены явно (Phase 3 группы, Phase 7 таблица, Phase 8 таблица тестовых файлов).

## Cross-check с корневым `goga-claude.md` (`goga-plan`-артефакт)

Корневой `goga-claude.md` (execution-plan, отдельный от этого arch-документа) не противоречит: оба
документа согласны, что архитектура — 16 клеточек, без изменений границ. Данный audit-раздел не меняет
и не отменяет ничего в корневом файле.

## Verification Checklist (для следующей стадии `goga-accept`)

- [x] Перезапустить `goga schema`/`goga lint` в окружении с доступным `goga` (build-образ
      `qarium/goga-golang-1.26:1.0` или Docker-режим afm) и сверить: 16 клеточек, `errors: 0`, граф
      Imports 1:1 с §Phase 3 выше, поле `usages` для 16 клеточек 1:1 с §Phase 7 таблицей. — **Готово**,
      см. Update в разделе «Ограничение окружения»: живой `goga lint`/`goga schema` подтвердил все цифры
      без расхождений (venv non-persistent, см. ограничение там же).
- [ ] Закрыть пробел `pkg/progress` — написать `.usages/` facade-файл (или явно обосновать, почему он
      не нужен, если `goga schema` покажет иную картину графа потребителей).
- [ ] Для каждой из 3 групп (bottom-up) выполнить manifest↔код сверку, `.usages`-актуальность и
      тест-покрытие через `goga-accept`, произвести 3 отчёта по формуле Acceptance Criteria задачи.
- [ ] Проверить дублирование dialog-protocol описания между `pkg/orchestrator` и `pkg/server` на предмет
      рассинхронизации.
- [x] Явно сверить annotations `pkg/mcp`, `pkg/orchestrator`, `pkg/proxy`, `pkg/docker` с `CLAUDE.md` по
      существу (не по ключевым словам), как того требует Acceptance Criteria #4 задачи. — **Готово для
      3 из 4 клеточек** (`pkg/docker` уже был сверен на предыдущем проходе), см. addendum ниже
      («Аддендум (стадия `brainstorm-review`) — досверка annotations vs CLAUDE.md для AC#4»).

---

## Аддендум (стадия `brainstorm-review`) — досверка annotations vs CLAUDE.md для AC#4

**Контекст.** Ревью этой стадии (`goga-review-arch`) обнаружило, что аудит-раздел выше сверил с
`CLAUDE.md` по существу только `pkg/docker` (Phase 8, Группа 2), а `pkg/mcp`, `pkg/orchestrator`,
`pkg/proxy` были лишь перечислены как работа для `goga-accept` — без обоснования, почему они не
сверены здесь же, притом что сверка annotations vs `CLAUDE.md` не требует чтения `.go`-кода (это
сравнение двух документов) и, в отличие от AC#1/#3/#5, не выходит за инвариант `goga-brainstorm`
(«никогда не читать implementation-код»). Все три CODEMANIFEST сопоставимого размера с `pkg/docker`
(107 строк): `pkg/mcp` — 127, `pkg/orchestrator` — 315, `pkg/proxy` — 83. Ниже — досверка.

**`pkg/mcp` vs CLAUDE.md → «File-Based Dialog Protocol».** Совпадает по существу:
- Путь и формат вопрос/ответ файлов (`<phase>.<id>.question.json`/`answer.json` в `AFM_STAGE_DIR`) —
  совпадает дословно.
- `AppendAnswer` annotation явно документирует, что эта функция НЕ атомарный критический путь (нет
  `O_EXCL`), и что реальная атомарная поставка ответа реализована отдельно в `pkg/server`
  (`handleDialogAnswer`, `os.OpenFile` с `O_EXCL`) — это точно соответствует границе ответственности
  из CLAUDE.md («Answer Delivery Guarantee») и корректно НЕ приписывает `pkg/mcp` чужую
  ответственность (соблюдена изоляция границ клеточки, см. `goga-cookbook` → Contract boundary
  isolation).
- Глобальная аннотация корректно делегирует orchestrator-детали (`startQuestionPoller`,
  `NotifyAnswer`) ссылкой на `pkg/orchestrator` (§6.14), не дублируя их здесь.
- Вердикт: **соответствует**, расхождений не найдено.

**`pkg/orchestrator` vs CLAUDE.md → «Agent Activity Tracking» / «Dialog protocol violation».**
Совпадает по существу:
- `Orchestrator` type-level Algorithm описывает `startQuestionPoller` (посекундный polling,
  `FindUnansweredQuestions` из `pkg/mcp`), `activeAgents`, и ветвление `NotifyAnswer` (FSM-переход,
  если агент активен; restart через `--resume` при неактивном) — семантически идентично CLAUDE.md,
  не только по ключевым словам.
- `detectDialogViolation` (упомянутая в CLAUDE.md функция обнаружения нарушения протокола) —
  проверено в коде: `func detectDialogViolation(...)` в `pkg/orchestrator/orchestrator.go:413`
  является неэкспортированной (строчная `d`). По правилу Go-фасада (`goga-cell-go`: «Public API =
  exported identifiers») она закономерно отсутствует в CODEMANIFEST — это не пробел, а корректное
  соблюдение facade-границы.
- Вердикт: **соответствует**, расхождений не найдено.

**`pkg/proxy` vs CLAUDE.md → «Built-in Reverse Proxy».** Совпадает по существу:
- `Transform` интерфейс (`Match`/`ServeHTTP`), `BuildTransforms` (`zai`: nil=автоопределение/
  true/false), `ZAITransform` Algorithm (5 шагов: inject stream:true → forward → накопить SSE
  message_start/content_block_start+delta/message_delta → вернуть единый JSON `message`) и
  Constraints (non-200 passthrough, SSE `error`/пустой ответ → HTTP 529) — совпадают дословно по
  смыслу с разделами «How the ZAI transform works» и списком Constraints из CLAUDE.md.
  `CreateShim` annotation (временный shell-скрипт `claude`, переопределяющий
  `ANTHROPIC_BASE_URL`, требование «claude должен быть в PATH») совпадает с CLAUDE.md → «Wrapper
  commands» и «Environment Variables».
- CLAUDE.md → «Known limitations» (`http.DefaultClient` без таймаута, `\n`-only SSE terminator,
  незаполненный `stop_sequence`, двойной слэш в `passthroughTo`) — это документированные баги
  текущей реализации, не архитектурные ограничения контракта; по `goga-cookbook`
  («Annotations must not contain implementation details») им корректно нет места в CODEMANIFEST.
  Не пробел.
- Вердикт: **соответствует**, расхождений не найдено.

**Итог по AC#4.** Все 4 клеточки (`pkg/docker`, `pkg/mcp`, `pkg/orchestrator`, `pkg/proxy`) теперь
явно сверены по существу с `CLAUDE.md`; расхождений не найдено ни в одной. Acceptance Criteria #4
задачи `docs/tasks/goga-claude.md` закрыт для целей этой ревью-стадии (сама по себе финальная
приёмка остаётся за `goga-accept`, как и для AC#1/#3/#5).

---

## Apply (стадия `apply`, 2026-07-05)

Материализация конкретных пробелов, выявленных audit-разделом выше. `goga` был установлен через
`pip install goga` в изолированный venv (см. Ограничение окружения — легитимный fallback-путь наряду
с Docker-образом); `goga connect claude` не выполнялся.

**До изменений:** `goga lint` → `cells: 16 errors: 0`; `goga schema` → `pkg/progress.usages: []`,
единственный потребитель — `pkg/executor` (тип `Logger`), подтверждая пробел из Phase 7.

**Изменённые файлы:**
- Создан `pkg/progress/.usages/logger_facade.md` — cell-level usage-файл для единственного реального
  потребителя (`pkg/executor`, вызывает `progress.NewLogger`/`Log`/`LogAction`/`LogStart`/`LogEnd` в
  `executor.go:226,340`), по формату/конвенции существующих `.usages/` файлов (`pkg/executor/.usages/runner_facade.md`,
  `pkg/docker/.usages/docker_privilege_drop.md`).

Других manifest↔код drift-пунктов, требующих правки CODEMANIFEST, в audit-разделе не зафиксировано —
полная построчная сверка явно отнесена к мандату `goga-accept` (см. «Существенное наблюдение» выше),
а не этой стадии. Никакая другая клеточка не менялась.

**После изменений:** `goga lint` → `cells: 16 errors: 0` (без регрессии); `goga schema` → 16 клеточек
(15 верхнего уровня + `pkg/web/dashboard` как child `pkg/web`, без изменений), `pkg/progress.usages`
→ `["logger_facade.md"]`. Корневой `goga-claude.md` и `docs/tasks/goga-claude.md` не противоречат —
счёт клеточек (16) и границы не менялись ни в одном из трёх документов.

**Вердикт: готово к передаче в `goga-accept`.** Единственный конкретный, установленный на уровне
схемы пробел (`pkg/progress` `.usages/`) закрыт и подтверждён живым `goga lint`/`goga schema`. Полная
manifest↔код сверка и оценка тестового покрытия по 3 группам (bottom-up), как и было решено в audit-
разделе, остаются работой `goga-accept`, не этой стадии.
