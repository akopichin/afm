...
- **`CoverageIssue`** — an aggregated gap descriptor with **structural attribution identity** (not a display string) and a **`count`**:
  `{kind: "unmetered"|"unpriced", attribution: {attribution_kind: "stage"|"run_overhead"|"unknown", stage_id?}, phase, channel, model, reason, count}`.
  - **Structural identity, not a sentinel (round-6 #4).** `safePathComponent` permits real stage ids literally named `run_overhead` or `—`, so grouping on a normalized display string would merge a genuine overhead record with a stage named `run_overhead` (and sum their counts). The identity is therefore `{attribution_kind, stage_id}` — `run_overhead` scope → `{kind:"run_overhead"}`, a stage record → `{kind:"stage", stage_id}`, an empty-owner non-overhead record → `{kind:"unknown"}`. The **human label** (`run_overhead` / the id / `—`) is derived separately for display (reusing `report.go`'s `recordLabel` logic), never used as the key.
  ...
  - **`count` semantics for consumers (round-6 #6):** every invocation total is `sum(issue.count)`, **never `len(issues)`** — two identical unpriced invocations are ONE issue with `count==2`, and `afm check`'s `coverageNote` must report `2 unpriced`, not `1`. Cost-tab rows show `×count`; the header tooltip is bounded (top N) and states hidden **groups** and hidden **invocations** as distinct numbers.
...
- **`Load` error is no longer ignored — exact exit/stream/shape contract (round-4 #7 / round-5 #5).** Today `afm check`/`report` swallow `accounting.Load` errors → a corrupt ledger reads as "No usage data". The precise contract, so shell automation is deterministic:
  ...
  |---|---|---|
  | missing `usage.jsonl` | cost columns show `—`/omitted, `No usage data` line on **stdout**, **exit 0** | title + FSM-derived stage rows (status/duration) + a `No usage data` note on **stdout**, no cost columns, **exit 0** |
  | corrupt (`ErrCorruptUsage`) | table renders (status/tokens where available), a `cost unavailable (ledger corrupt)` warning on **stderr**, **exit 3** (a defined non-zero) | title + FSM-derived stage rows + a `Cost unavailable — ledger corrupt` warning; **no invented token/cost totals**; warning on **stderr**, **exit 3** |
  | normal | full cost, exit 0 | full report, exit 0 |
  `report`'s `RenderMarkdown` must therefore stop early-returning bare `No usage data` for `led==nil` — it renders the title + state-derived stage rows in both degraded cases, differing only in the note. Golden tests assert **stdout, stderr, and exit code separately** for missing / corrupt / normal; the normal golden preserves `Rates used` provenance + reported-vs-estimated mismatch.
