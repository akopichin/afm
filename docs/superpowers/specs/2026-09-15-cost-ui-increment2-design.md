...
- **`CoverageIssue`** — a gap descriptor with a normalized **`attribution`** and a **`count`**:
  `{kind: "unmetered"|"unpriced", attribution, phase, channel, model, reason, count}`.
  - **`attribution` (round-5 #2), not a bare `stage`.** An overhead record has `StageID == ""` but `Scope == run_overhead`; a bare `stage` would render an empty owner. `attribution` is the normalized owner — the stage id, or `run_overhead`, or `—` — computed exactly like `report.go`'s existing `recordLabel` (reuse it, one source). CLI and UI both consume `attribution`.
  - **Aggregated, not per-invocation (round-5 #3).** The dashboard read model returns issues **grouped by the stable key `{kind, attribution, phase, channel, model, reason}` with a `count`** — a long run with many repeated unpriced/retry invocations must not grow `/api/status` (polled every 3s) or the tooltip/coverage line linearly. Order is deterministic (sorted by attribution, then phase). The header tooltip shows a **bounded** summary (e.g. top N + "…and K more"); the Cost tab coverage line lists the grouped rows. (`afm report` MAY still walk raw records for its own detailed appendix — but the shared `[]CoverageIssue` in the DTO is the aggregated form.)
...
2. **Rail per-stage cost** — a quiet, right-aligned mono figure in the row's trailing slot (`.stage-actions`), left of the kebab; no chip/background. **One helper decides it, with access to Stage AND accounting availability/presence (round-4 #3):**
   ...
   2. `stage.cost == nil` AND **`accounting.supported && accounting.health === 'ok'`** AND status ∈ `COST_PENDING_STATUSES` → `…`. The gate is exactly `health === 'ok'`, NOT "not hard-unavailable" (round-5 #1): once the `Store` latches `unavailable` (any `unavailable`, incl. `unavailable,true` where other stages already have a snapshot) it is permanently disabled and will produce no new record, so a pending stage must show nothing — a `…` there is a false promise. Known cost of *other* stages keeps showing (their `stage.cost != nil`).
...
- **Accounting**: `Coverage` from counts — **single unpriced → `none`** (not partial), priced+unpriced → `partial`, zero-rate full → `full`/`$0.0000`, zero-rate+unpriced → `partial`/`$0.0000+`; `DisplayCost` exact strings incl. tiny full `<$0.0001` and **tiny partial `<$0.0001+`**; `priced_invocations` correct; `CoverageIssue` — **`attribution` is `run_overhead` for an overhead gap, the stage id otherwise** (unpriced AND unmetered overhead records, not just a multi-model stage); **aggregation groups identical gaps into one row with `count==N`** (N repeated unpriced/retry invocations → one issue, `count==N`, coverage still correct), deterministically ordered.
...
- **CLI**: `afm check`/`report` render `DisplayCost` (partial `$X+`); coverage note/section from `CoverageIssue` (using `attribution`, so an overhead gap reads `run_overhead`) while **keeping Rates-used + mismatch subsections**; the corrupt/missing/normal matrix is asserted with **stdout, stderr, and exit code checked separately** (missing → stdout `No usage data`, exit 0; corrupt → stderr warning, **exit 3**, no invented totals; normal → exit 0); `report`'s degraded shape renders title + FSM-derived stage rows, not a bare `No usage data`.
