...
**Реализованное поведение (уточнение после review #10 — исходный текст ниже описывал "выбрать самый свежий completed run, затем hard-error при несовпадении stage-set"; это НЕ то, что реализовано).** Резолвер принимает текущее множество stage ID флоу (`wantStages`) и использует его как ФИЛЬТР кандидатов, а не как проверку постфактум:
...
4. Обычный incomplete run пропускать (позволяет backfill последнего completed, даже если поверх начат новый).
5. Для каждого completed-кандидата сравнить его точное множество стадий (`rs.Stages`, все со статусом `done`) с `wantStages`: **точное совпадение** (не подмножество/надмножество) — результат; **несовпадение — кандидат молча пропускается**, резолвер переходит к следующему, более старому кандидату. Никакой hard-error здесь не возникает.
6. Corrupt matching log **не** скрывать fallback-ом: вернуть контекстную `ErrCorruptLog` (как `FindLatestRunForStage`) — но ТОЛЬКО если matching (anchored-prefix) кандидат битый; сам факт битого лога не заставляет пропустить его молча.
7. Отсутствующий/нечитаемый `events.jsonl` — ошибка кандидата (не hard-error всей команды, кандидат просто пропускается), не доверять `state.json`.
8. Если ни один кандидат не даёт точного совпадения — `ErrNoRun` ("no matching run found").

**Последствие (сознательный компромисс, review #10):** команда может молча анализировать СУЩЕСТВЕННО более старый run, если самый свежий completed run флоу имеет другую топологию стадий (стадию добавили/удалили/переименовали со времени этого свежего рана). Резолвер не сообщает, что более свежий run существовал, но был пропущен из-за несовпадения — только явный `--run <id>` даёт полный контроль над выбором конкретного рана (и тогда несовпадение stage-set — см. §5.3 — уже hard error, а не тихий пропуск). Выбор сделан намеренно: "select-then-hard-error-on-mismatch" заблокировал бы backfill каждый раз, когда топология флоу меняется, а именно такой backfill (добавить `memory`/`reflect` задним числом на уже отредактированный флоу) — основной сценарий команды. Пользователь, которому важно проверить, какой именно run был выбран, видит его id в stdout (`using current flow definition and current prompts for historical run <id>`) и в манифесте попытки.

Explicit `--run`: принимается только безопасный basename — запретить `/`, `\`, абсолютный путь, `.`/`..` и любое значение, где `filepath.Base(v) != v`. После `Join(runsDir(), id)` проверить существование директории и anchored принадлежность имени к `flow.Name`. Explicit `--run` НЕ проходит через фильтр `wantStages` резолвера (он обходит `FindLatestCompletedRunDir` целиком) — completeness (`AllDone`) и exact stage-set match проверяются НЕЗАВИСИМО после резолва директории, и несовпадение здесь — hard error с двумя отсортированными списками (см. §5.3), а не тихий пропуск.
...
Сравнить **точное** множество stage ID из replay с текущим `flow.Stages`. При отсутствии `--run` это сравнение уже встроено в фильтр резолвера §5.1 (несовпадение → кандидат пропущен, не ошибка); при явном `--run` (обходит фильтр) — независимая проверка сразу после резолва директории:

- missing в YAML или extra в YAML — hard error с двумя отсортированными списками (актуально для explicit `--run`; для default-резолвера см. §5.1 — несовпадающий run туда просто не попадает как результат);
