...
- 13 cell-level `.usages/`-файлов созданы изначально; 14-й (`pkg/progress/logger_facade.md`) добавлен
  позже стадией `apply` этого же прогона, закрывая пробел из аудита (см. «`.usages/` Update»).
...
Изначальные 13 cell-level `.usages/`-файлов созданы стадией `goga-apply` (перечислены в §6.17
arch-плана). Свёрка с оттрассированными контрактами: каждый описывает фасад потребителя («как вызывать
клеточку»), API совпадают с `CODEMANIFEST`. Дополнений/обновлений по ним не требуется — состав API
потребителя не менялся (реверс-инжиниринг as-is).

С тех пор аудит-стадии этого же прогона (`brainstorm`/`brainstorm-review`) обнаружили дополнительный
пробел: `pkg/progress` имел внутреннего потребителя (`pkg/executor`), но не имел cell-level `.usages/`.
Стадия `apply` закрыла его, создав `pkg/progress/.usages/logger_facade.md`. Итого 14 файлов.

Проверено для нового файла: `pkg/executor/executor.go` — единственный внутренний потребитель
`pkg/progress` (`grep -rl` по всему дереву, исключая тесты и worktrees, даёт единственное совпадение).
Документированный API (`NewLogger`, `Log`, `LogAction`, `LogStart`, `LogEnd`, `Close`) совпадает с
методами entity `Logger` в `pkg/progress/CODEMANIFEST`. Методы entity `Lock` (`Lock`/`TryLock`/`Unlock`)
в файле не описаны — обоснованно: у `Lock` нет внешнего потребителя за пределами клеточки, платформенно-
специфичная реализация (`x_sys_windows`) — внутренняя практика, а не потребительский фасад.

**CODEMANIFEST `Usages`-ссылок на собственные `.usages/` не добавлено** (`.usages/` — документация для
потребителя, не источник контракта; проверено: ни одна клеточка не ссылается на собственный `.usages/`).
...
| `pkg/progress` | `logger_facade.md` | new — закрывает пробел, найденный аудитом `brainstorm`/`brainstorm-review`; единственный потребитель `pkg/executor` |

`tools/setstatuslinter` и `cmd/afm` не имеют `.usages/` (нет внутренних потребителей — терминальные
вершины графа зависимостей, проверено, §6.17 arch-плана).
