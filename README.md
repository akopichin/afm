...
Для запуска `claude` в интерактивных стадиях добавляются флаги `--print --output-format stream-json --verbose --dangerously-skip-permissions` (`--verbose` обязателен для stream-json в Claude Code 2.1.x).

> Если интерактивный агент по ошибке запишет `question.json` **вне** `$AFM_STAGE_DIR` (известный баг некоторых моделей — путь строится из CWD вместо env-переменной), стадия НЕ падает: поллер обнаруживает запись и автоматически перемещает файл вопроса в stage-директорию, диалог появляется в UI, а по неверному пути оставляется симлинк на ответ — чтобы bash-цикл агента нашёл ответ там, где ищет. В логе видно `INFO: relocated misplaced question`.
