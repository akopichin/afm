...
**Docker mode.** Rebuild honors Docker mode exactly like `afm run`: if Docker is enabled it re-execs into the container before taking any locks, mounting only the memory-agent command (`client.command`) — no dashboard, browser, or file-browser is started. An external, absolute `memory.path` that's reachable inside the container only through a read-only `extra_mounts` entry is rejected up front (memory must be writable), so add it as a writable mount or keep `memory.path` inside the project.
