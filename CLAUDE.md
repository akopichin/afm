...
- **`runWithRetry` (`retry.go`) — тот же класс гонки, вторая точка входа, найдено отдельным аудитом "нет ли протечек в FSM".** `SpawnAgent` (`concurrency.Manager`) ставит горутину в очередь за семафором (`max_parallel`) ДО того, как `runWithRetry` вообще стартует и регистрирует `interruptChans` — если стадию Pause'нули (снова) прямо в это окно ожидания слота (типичный сценарий: `Continue()` зарезюмил стадию в уже полный `max_parallel`-бакет, юзер тут же жмёт Pause второй раз), сигналить ещё нечему (`interruptChans.Load` вернёт `false`), а когда семафор освободится — горутина всё равно запустит настоящего агента поверх уже paused-стадии. В отличие от `withBeforeHook` (защищает только "свежую активацию": script/autonomous/implementation через `startReadyStages`/`retryStage`), эта гонка достижима и через resume-пути в обход `withBeforeHook` (`resumeStageAtStatus`'s `*WithFeedback`-спавны, `runPlanningAgent` из recovery/retry-веток) — но ВСЕ агентские фазы (planning/implementation/review/autonomous, включая `*WithFeedback`) без исключения проходят через `runWithRetry` (8 call site'ов в `agents.go`), так что проверка `currentStatus == paused` в самом начале `runWithRetry`, до регистрации `interruptChans`, закрывает гонку единообразно для любой точки входа, а не требует патчить каждый вызывающий код отдельно.
